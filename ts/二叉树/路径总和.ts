...
var hasPathSum = function (root: TreeNode, sum: number) {
  ...
  const dfs = function (node: TreeNode|null, cur: number) {
    ...
    if (node.left === null && node.right === null) {    //判断是叶节点
      if (cur+<number>node.value === sum) {
        state = true;
      }
      return;
    }
    dfs(node.left, cur + <number>node.value);
    dfs(node.right, cur + <number>node.value);
  }
  dfs(root, cur);
  return state;
};
